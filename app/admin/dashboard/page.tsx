...
export default async function AdminDashboardPage({ searchParams }: PageProps) {
  ...
  const allStatusRows = await prisma.order.findMany({
    distinct: ["status"],
    select: {
      status: true,
    },
    ...
    <div className="min-h-screen bg-gray-50 p-6 md:p-8">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">订单报表</h1>
            <p className="mt-2 text-sm text-gray-500">
              默认显示当前月份，可按月份、状态或自定义期间筛选
            </p>
          </div>

          <Link
            href="/admin/login"
            className="inline-flex w-fit items-center rounded-xl border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm transition hover:bg-gray-100"
          >
            返回登录页
          </Link>
        </div>
